...
class Gaussian(DP):
    ...
    def add_noise(self, syn_data, noise_multiplier):
        ...
        ] + np.random.normal(scale=noise_multiplier, size=len(syn_data.data_frame))
        return syn_data


    def sampling_prob(self, sec_matrix, p, r, num_samples):
        """
        Solve linear program to get sampling distribution.
        :param sec_matrix: The secret-containing matrix (N, J)
        :type sec_matrix: np.ndarray
        :param p: prior probability vector (J, )
        :type p: float
        :param r: posterior probability vector (J, )
        :type r: float
        :param num_samples: Batch size
        :type num_samples: int
        """
        sec_matrix = np.asarray(sec_matrix, dtype=float)
        N, J = sec_matrix.shape
        ...
        r_clip = np.clip(r, eps, 1 - eps)

        den = norm.ppf(1.0 - p_clip) -  norm.ppf(1.0 - r_clip) # denominator
        if np.any(den <= 0):
        ...
        w = res.x
        if np.sum(w) == 0.0:
            raise ValueError("Error: LP failed to select private data.")
        
        w = w / np.sum(w)
        B = 1.0 / np.max(w)
        if float(num_samples) > B:
            w = w * float(B)
            execution_logger.warning(f"Batch size for SecDP change to {B}.")
        else: w = w * float(num_samples)
    ...
    def get_noise_multiplier(
        ...
        :param p: prior probability vector
        :type p: float
        :param r: posterior probability vector
        :type r: float
        :param num_steps: The number of steps
        :type num_steps: int
        :param min_noise_multiplier: The minimum noise multiplier to search for, defaults to 1e-1
        ...
        :param max_noise_multiplier: The maximum noise multiplier to search for, defaults to 500
        :type max_noise_multiplier: float, optional
        :param mode: The mode of the DP mechanism, defaults to "secdp"
        :type mode: str, optional
        """
        w = np.asarray(w, dtype=np.float64)
        sec_matrix = np.asarray(sec_matrix, dtype=np.float64)
        rho = np.sum(sec_matrix * w[:, None], axis=0) 
        ...
        den = norm.ppf(1.0 - p_clip) -  norm.ppf(1.0 - r_clip)
        if self._mode == "secdp":
            sigma = (np.sqrt(num_steps) * rho) / den
            noise_multiplier = float(np.max(sigma))
        elif self._mode == "gdp":
            smapling_prob = np.sum(w) / sec_matrix.shape[0]
            sens = sec_matrix.sum(axis=0)
            sigma = smapling_prob * sens * np.sqrt(num_steps) / den
            noise_multiplier = float(np.max(sigma))
        else:
            raise ValueError(f"Unknown mode: {self._mode}")

        return float(np.clip(noise_multiplier, min_noise_multiplier, max_noise_multiplier))
